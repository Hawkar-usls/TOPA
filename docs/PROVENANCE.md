...
Exact source commits and SHA-256 hashes of managed imports are generated into `registry/UPSTREAM-SNAPSHOT.json` and `registry/TOPA-CORPUS-INDEX.json`.

## Habitat and methodological integrations

TOPA also carries local handoff/boundary artifacts under `integrations/`. These are not automatically exact source mirrors and do not receive evidence authority merely by being connected.
...
Some Hawkar-owned repositories are upstream-derived working/reference copies. Ownership of a GitHub fork/copy does not transfer authorship of the base research or code.

TOPA evaluates JANUS/Hawkar-specific work only through explicit modifications, receipts or local handoff artifacts.
...
`.github/workflows/bootstrap-topa.yml` performs a reproducible managed snapshot import and now also validates the Habitat contract before ingestion.
...
- indexes the complete local `.janus`, `protocols`, `corpus`, `research`, `integrations` and registry handoff surface;
- commits only when the managed snapshot changes.

The workflow validates repository state and provenance. It does **not** score scientific truth.
