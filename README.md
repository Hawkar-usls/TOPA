...
For mathematics, the same discipline becomes:

```text
FREEZE CLAIM
  → NORMALIZE DEFINITIONS + PARAMETERS
  → VERIFY ENCODING / OBJECT IDENTITY
  → SEARCH PROOF AND COUNTEREXAMPLE ROUTES
  → CHARGE FULL ALGORITHMIC + CERTIFICATE COST
  → CLASSIFY PROVED / REFUTED / OPEN
  → ASCEND AND REATTACK

FINITE TEST != UNIVERSAL ASYMPTOTIC THEOREM
