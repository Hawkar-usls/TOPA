...
class TOPAEpistemicRouter:
    ...
    def assess(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        source_pointers = _strings(
            _as_list(record.get("source_pointers"))
            or _as_list(record.get("source"))
        )
        ...
        if not raw_claim_text:
            errors.append("MISSING_RAW_CLAIM_TEXT")
        ...
        # Heuristic authority is rejected, not merely ignored.
        ...
        # Authority-class-specific admission.
        if evidence_class == "SOURCE_FACT":
            if not source_pointers:
                errors.append("SOURCE_FACT_REQUIRES_SOURCE_POINTER")
            if not provenance_channel:
                errors.append("SOURCE_FACT_REQUIRES_PROVENANCE_CHANNEL")
        ...
        if evidence_class == "STATISTICAL_INFERENCE":
            if statistical_model is None:
                ...
                required = ["target", "model", "uncertainty"]
                missing = [key for key in required if not statistical_model.get(key)]
                if missing:
                    errors.append("STATISTICAL_MODEL_MISSING:" + ",".join(missing))
        ...
        # A probability is scientific metadata only with an explicit model/calibration contract.
        if probability_model is not None:
            required = ["model", "conditioning", "estimation_or_calibration", "validation", "uncertainty"]
            missing = [key for key in required if not probability_model.get(key)]
            if missing:
                errors.append("PROBABILITY_MODEL_MISSING:" + ",".join(missing))
        ...
        if prediction is not None:
            ...
            required_freeze = [
                "prediction_timestamp", "deadline_or_window", "success_criterion",
                "failure_criterion", "analysis_rule",
            ]
            ...
            allowed_scores = {
                "PASS", "FAIL", "PARTIAL_WITH_PREDECLARED_TOLERANCE",
                "UNSCORABLE_TOO_VAGUE", "UNRESOLVED_MISSING_OUTCOME_DATA",
            }
            if score and score not in allowed_scores:
        ...
        return {
            ...
            "heuristic_authority_used": False if not any(code.startswith(("UNCALIBRATED", "HEURISTIC", "SURVIVOR", "MODEL_VOTE", "CONSENSUS", "VISUAL")) for code in errors) else True,
            "errors": errors,
            "warnings": warnings,
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "strict_science_gate": "Hawkar-usls/TOPA:protocols/TOPA_STRICT_SCIENCE_GATE_V1_0.json",
...
def self_test() -> None:
    ...
    source_fact = router.validate({
        ...
        "raw_claim_text": "Instrument log contains event E.",
        "claim_text": "Source S records event E.",
        "claim_kind": "OBSERVED",
        "evidence_class": "SOURCE_FACT",
        "provenance_channel": "INSTRUMENT_RECORD",
        "firsthand_status": "NOT_APPLICABLE",
        "source_pointers": ["source://instrument-log"],
    ...
    legacy = router.assess({
        ...
        "claim_text": "Legacy survivor label.",
        "claim_kind": "HYPOTHESIS",
        "evidence_class": "HYPOTHESIS_ONLY",
    ...
    prediction = router.validate({
        "claim_id": "C-5",
        "origin_agent_id": "SCOUT_E",
        "claim_text": "Event X will occur before D.",
        "claim_kind": "HYPOTHESIS",
        "evidence_class": "HYPOTHESIS_ONLY",
        "alternative_hypotheses": ["no event"],
        "falsification_tests": ["frozen deadline check"],
        "status": "HYPOTHESIS_ONLY",
        "prediction": {
            "frozen": True,
            "scored": True,
            "prediction_timestamp": "2026-08-23T00:00:00Z",
            "deadline_or_window": "2026-08-24",
            "success_criterion": "X observed",
            "failure_criterion": "X absent",
            "analysis_rule": "binary presence/absence from frozen source",
            "score": "FAIL",
        },
    })
