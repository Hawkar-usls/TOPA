...
there are polynomial-size input ZDDs whose result requires exponential ZDD size, and the blow-up persists for every element order.

Precision boundary: this source lower bound is against BDD/ZDD family representations and the source family-algebra operation. It is **not** an unconditional lower bound against arbitrary B2 circuits, nor by itself a lower bound for PF1's exact Boolean factor representation. It closes only the shortcut “ordinary BDD/ZDD family-operation canonicalization is automatically a universal polynomial quotient.”

Therefore:

`BDD_ZDD_CANONICALITY != UNIVERSAL_POLYNOMIAL_QUOTIENT_SIZE`.
...
`PF2_BDD_ZDD_UNIVERSAL_POLY_QUOTIENT_SHORTCUT = REFUTED_IN_SOURCE_REPRESENTATION_SCOPE`
